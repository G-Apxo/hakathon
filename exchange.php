<!-- Header -->
<?php include_once 'header.php';?>
<!-- End Header --> 
<!-- Content -->
<div id="content">
   <!-- Seo Secore -->
   <section class="bg-parallax seo-secore padding-top-100 padding-bottom-50" no-repeat;">
      <div class="container">
         <!-- Tittle -->
         <div class="heading-block text-dark text-left margin-bottom-80">
            <h2 class="text-dark" style="font-size:40px;">არაფორმარული განათლება<span style="color:#00a561; font-size:40px;"> შენს არეალში </span></h2>
         </div>
         <!-- Form -->
         <form>
            <ul class="row">
               <li class="col-md-4">
                  <div class="form-group">
                     <label for="exampleFormControlSelect1">ფაკულტეტი</label>
                     <select class="form-control" id="exampleFormControlSelect1">
                        <option>ყველა</option>
                        <option>ფსიქოლოგია</option>
                        <option>სოციოლოგია</option>
                        <option>სამედიცინო</option>
                        <option>ფინანსები</option>
                     </select>
                  </div>
               </li>
               <li class="col-md-4">
                  <div class="form-group">
                     <label for="exampleFormControlSelect1">ლოკაცია</label>
                     <select class="form-control" id="exampleFormControlSelect1">
                        <option>ყველა</option>
                        <option>თბილისი</option>
                        <option>ქუთაისი</option>
                        <option>გორი</option>
                        <option>ბათუმი</option>
                        <option>ხაშური</option>
                     </select>
                  </div>
               </li>
               <li class="col-md-4 margin-top-10">
                  <button type="submit" class="btn btn-orange">ძებნა</button>
               </li>
            </ul>
         </form>
      </div>
   </section>
   <!-- Google Front Page -->
   <section class="front-page padding-bottom-100">
      <div class="container padding-bottom-50 padding-top-50">
         <div class="row">
            <div class="col-md-6 custom_hov" style="border: solid 2px #a8d806;padding-top: 35px;padding-bottom: 35px;border-radius: 35px;">
               <div class="col-md-4" style="display: flex;justify-content: center;align-items: center;"> <img class="img-responsive " src="https://scontent.fkut1-1.fna.fbcdn.net/v/t1.18169-9/13516706_508457376027922_6490481796783975719_n.jpg?_nc_cat=106&ccb=1-5&_nc_sid=09cbfe&_nc_ohc=TWeLECtUBgoAX-qk_go&_nc_ht=scontent.fkut1-1.fna&oh=ae6d488cc2ca622982c70b7ef633b766&oe=61AFEA95" alt=" " style="width:100%"> </div>
               <div class="col-md-8">
                  <!-- Tittle -->
                  <div class="heading-block text-left" style="margin-bottom:5px">
                     <h2>PITA - ქუთაისის ახალგაზრდული ცენტრი </h2>
                  </div>
                  <div class="row">
                     <div class="col-md-6">
                        <ul class="list-style">
                           <li>
                              <p><img src="images/price-tag.png" alt="" style="width:10%"> ევროპა: </p>
                           </li>
                        </ul>
                     </div>
                     <div class="col-md-6">
                        <ul class="list-style">
                           <li>
                              <p><img src="images/check.png" alt=""style="width:10%">ჩარიცხვის %: </p>
                           </li>
                        </ul>
                     </div>
                  </div>
                  <!-- Buttons --> 
                  <div class="col-md-12">
                     <div class="row">
                        <div class="col-md-6">
                        </div>
                        <div class="col-md-6" style="display: flex;justify-content: end;margin-top: -7vh;">
                           <a href="#." class="btn margin-top-20">გაიგე მეტი</a>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
            <div class="col-md-6" >
               <div class="col-md-4" style="display: flex;justify-content: center;align-items: center;"> <img class="img-responsive " src="https://scontent.fkut1-1.fna.fbcdn.net/v/t1.18169-9/13516706_508457376027922_6490481796783975719_n.jpg?_nc_cat=106&ccb=1-5&_nc_sid=09cbfe&_nc_ohc=TWeLECtUBgoAX-qk_go&_nc_ht=scontent.fkut1-1.fna&oh=ae6d488cc2ca622982c70b7ef633b766&oe=61AFEA95" alt=" " style="width:70%"> </div>
               <div class="col-md-8">
                  <!-- Tittle -->
                  <div class="heading-block text-left" style="margin-bottom:5px">
                     <h2>PITA - ქუთაისის ახალგაზრდული ცენტრი </h2>
                  </div>
                  <div class="row">
                     <div class="col-md-6">
                        <ul class="list-style">
                           <li>
                              <p><img src="images/price-tag.png" alt="" style="width:10%"> სწავლის საფასური: </p>
                           </li>
                           <li>
                              <p><img src="images/check.png" alt=""style="width:10%">ჩარიცხვის %: </p>
                           </li>
                        </ul>
                     </div>
                     <div class="col-md-6">
                        <ul class="list-style">
                           
                        </ul>
                     </div>
                  </div>
                  <!-- Buttons --> 
                  <div class="col-md-12">
                     <div class="row">
                        <div class="col-md-6">
                        </div>
                        <div class="col-md-6" style="display: flex;justify-content: end;">
                           <a href="#." class="btn margin-top-20">გაიგე მეტი</a>
                        </div>
                     </div>
                  </div>
               </div>
            </div>
         </div>
      </div>
   </section>
</div>
<!-- End Content --> 
<!-- Footer -->
<?php include_once 'footer.php';?>
